...

{
	...
	this->ebulletTex.loadFromFile("Textures/eBullet.png");
	...
	// Init Menu
	this->menuMusic.openFromFile("Audio/menu.ogg");
	menuMusic.setLoop(true);
	menuMusic.setVolume(10.f);
	menuMusic.play();

	// Init BGM
	this->background.openFromFile("Audio/spongebob.ogg");
	background.setLoop(true);
	background.setVolume(0.f);
	background.play();

	// Init Game over
	this->over.openFromFile("Audio/over.ogg");
	over.setVolume(10.f);

	// Init choose sound
	this->choose.openFromFile("Audio/choose.ogg");
	choose.setVolume(10.f);

	// Init shoot sound
	this->shootBuffer.loadFromFile("Audio/Pew.ogg");
	this->shootSound.setBuffer(shootBuffer);
	this->shootSound.setVolume(20.f);

	// Init end sound
	this->endBuffer.loadFromFile("Audio/boom.ogg");
	this->endSound.setBuffer(endBuffer);
	endSound.setVolume(10.f);

	// Init enemy shoot sound
	this->eshotBuffer.loadFromFile("Audio/quack.ogg");
	...
	// Init score texture
	this->endGamePic.loadFromFile("Textures/hs.png");
	this->endGamePic.setSmooth(true);
	this->endGame.setTexture(endGamePic);
	this->endGame.setPosition(0.f, 0.f);

	// Init menu
	this->logoCnt = 0.f;
	this->logoDisplayMax = 3.f;
	...
	this->endScoreText.setFillColor(Color::White);

	// Background
	this->backgroundSpeed = Vector2f(-3.f, 0.f);

	// Init Textbox
	textbox.Setup(1, 25, 700, sf::Vector2f(600, 0));
	...
	this->doctor.loadFromFile("Textures/doctor.png");
	this->nurse.loadFromFile("Textures/nurse.png");
			...
			case Keyboard::Return:
				if (gameIsOver && !isLOGO)
				{
					...
					case 0: // New game
						...
						menuMusic.stop();
						background.play();
						textbox.Add("Welcome to blood vessel!\nPress K to shoot\nPress W A S D to move.\nLet's pass away together!!!", true);
						break;
					case 1: // Credits
						isDrawCredits = true;
						break;
...
void Game::Update(float dt)
{
	...
	if (!isMenu)
	{
		...
		else if (!gameIsOver)// Game starts here
		{
			// Calculate Index
			CalculateIndex(30);
...
{
	if (player.score % 300 == 0 && player.score != 0 && !generateBoss)
	{
...
void Game::PlayerStateUpdate(Player& player)
{
	...
	if (player.HP == 0)
	{
		...
		this->endScoreText.setString("YOUR SCORE : " + std::to_string(player.score));
...
void Game::TextboxUpdate(const float& dt)
{
	...
	{
		this->textbox.Add("You've reached " + std::to_string(player.score) + "pts, watch out for the VIRUS!!", true, &doctor);
		this->isHundred = true;
	}

	if (havePickedHP && !isStory)
	{
		this->textbox.Add("You've picked one HP Buff: health +1.     Your HP: " + std::to_string(player.HP), true, &nurse);
		this->havePickedHP = false;
	}

	if (pickedBulletBuff && !isStory)
	{
		this->textbox.Add("You've picked one Bullet Buff: Your shooting mode: strafet", true, &nurse);
